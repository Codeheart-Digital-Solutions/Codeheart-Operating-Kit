...
func TestLegacyV1UpgradeDryRunAndAtomicApply(t *testing.T) {
	if version.Version != "0.1.29" {
		t.Fatalf("test requires release version 0.1.29, got %s", version.Version)
	}
	...
	code := RunUpgrade([]string{root, "--version", "0.1.29", "--catalog", catalog, "--installed-binary", installed, "--dry-run", "--json"}, &preview, &bytes.Buffer{})
	if code != 0 {
	...
	if result["status"] != "planned" || payload["source_lock_schema"] != float64(1) || payload["target_lock_schema"] != float64(2) || payload["installed_version"] != "0.1.19" || payload["target_version"] != "0.1.29" || payload["migrated_lock"] != true {
	...
	code, applyOutput := runApprovedUpgrade(t, []string{root, "--version", "0.1.29", "--catalog", catalog, "--installed-binary", installed, "--yes", "--json"})
	if code != 0 {
	...
	if state.AsInt(lock["schema_version"]) != 2 || state.AsString(lock["kit_version"]) != "0.1.29" || state.AsInt(lock["state_generation"]) != 1 {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, dryRun := range []bool{true, false} {
				...
				t.Run(mode, func(t *testing.T) {
					...
					_, result, err := upgradeOperation(root, "0.1.29", filepath.Join(t.TempDir(), "unused-catalog.json"), writePreviousBinary(t), dryRun)
...
func TestLegacyV1UpgradeForwardAndTargetAuthorityPreconditions(t *testing.T) {
	...
	for name, mutate := range map[string]func([]string) []string{
		"wrong target":           func(args []string) []string { return replaceArgument(args, "--target-version", "0.1.30") },
...
func TestLegacyV1UpgradeFailedTargetReconcileRestoresBinaryAndState(t *testing.T) {
	...
	code, output := runApprovedUpgrade(t, []string{root, "--version", "0.1.29", "--catalog", catalog, "--installed-binary", installed, "--yes", "--json"})
	if runtime.GOOS == "windows" {
		if code != 0 {
...
func TestCurrentLockV2UpgradeBehaviorAndNoLegacyRepairSyncBypass(t *testing.T) {
	...
	installed := writePreviousBinary(t)
	catalog, _ := writeUpgradePack(t)
	if code, output := runApprovedUpgrade(t, []string{root, "--version", "0.1.29", "--catalog", catalog, "--installed-binary", installed, "--yes", "--json"}); code != 0 {
		t.Fatalf("v2 upgrade exit=%d output=%s", code, output)
	}
	waitForUpgrade(t, root, installed)
	upgraded, _ := lockfile.ReadLock(root)
	if state.AsInt(upgraded["schema_version"]) != 2 || state.AsString(upgraded["kit_version"]) != "0.1.29" || state.AsInt(upgraded["state_generation"]) != 2 || state.AsInt(state.Map(upgraded["last_operation"])["previous_generation"]) != 1 {
