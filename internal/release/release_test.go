...
func TestCatalogAndPackVerification(t *testing.T) {
	root := t.TempDir()
	archive, asset := writePackFixture(t, root, packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", includeBinary: true, reconcileSucceeds: true})
	catalogPath := writeCatalogFixture(t, root, asset)
	catalog, err := LoadCatalog(catalogPath)
	if err != nil {
		t.Fatal(err)
	}
	selected, err := catalog.Select("0.1.21", "macos-universal")
	if err != nil {
		t.Fatal(err)
	}
	...
	verified, err := VerifyPack(fetched, filepath.Join(root, "extract"), selected, VerifyOptions{Version: "0.1.21", Platform: "macos-universal", Command: "codeheart-operating-kit"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPackVerificationRejectsInvalidEvidence(t *testing.T) {
	tests := []struct {
		...
		{name: "missing-binary", options: packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit"}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "binary checksum"},
		{name: "payload-checksum", options: packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", includeBinary: true, corruptPayload: true}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "payload checksum"},
		{name: "wrong-version", options: packFixtureOptions{version: "9.9.9", platform: "macos-universal", command: "codeheart-operating-kit", includeBinary: true}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "pack version"},
		{name: "wrong-platform", options: packFixtureOptions{version: "0.1.21", platform: "windows-x64", command: "codeheart-operating-kit", includeBinary: true}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "pack platform"},
		{name: "wrong-command", options: packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "other", includeBinary: true}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "jsonschema validation"},
		{name: "traversal", options: packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", includeBinary: true, traversal: true}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "unsafe archive path"},
		{name: "symlink", options: packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", includeBinary: true, symlink: true}, version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", contains: "symbolic link"},
	}
...
func TestCatalogRejectsChecksumMismatchDuplicateAndUnknownFields(t *testing.T) {
	root := t.TempDir()
	archive, asset := writePackFixture(t, root, packFixtureOptions{version: "0.1.21", platform: "macos-universal", command: "codeheart-operating-kit", includeBinary: true})
	...
	catalog.Catalog.Assets[0].Name = "../codeheart-operating-kit-0.1.21-macos-universal.zip"
	if _, err := catalog.Select(asset.Version, asset.Platform); err == nil {
		t.Fatalf("unsafe catalog asset name accepted")
	}
	invalid := filepath.Join(root, "invalid.json")
	if err := os.WriteFile(invalid, []byte(`{"schema_version":1,"version":"0.1.21","assets":[],"unexpected":true}`), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestHandoffTamperingAndFailedReconcileRestorePreviousBinary(t *testing.T) {
	...
	archive, asset := writePackFixture(t, root, packFixtureOptions{version: "0.1.21", platform: platform, command: "codeheart-operating-kit", includeBinary: true, nativeBinary: true})
	pack, err := VerifyPack(archive, filepath.Join(root, "extract"), asset, VerifyOptions{Version: "0.1.21", Platform: platform, Command: "codeheart-operating-kit"})
	if err != nil {
		t.Fatal(err)
	}
	...
	handoff, err := NewHandoff(prepared, root, target, "0.1.20")
	if err != nil {
		t.Fatal(err)
	}
...
func TestVersionDirection(t *testing.T) {
	if err := RequireForwardUpgrade("0.1.20", "0.1.21"); err != nil {
		t.Fatal(err)
	}
	for _, target := range []string{"0.1.20", "0.1.19", "invalid"} {
		if err := RequireForwardUpgrade("0.1.20", target); err == nil {
